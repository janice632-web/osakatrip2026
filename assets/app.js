...
document.querySelectorAll('[data-page]').forEach(a=>a.addEventListener('click',e=>{
 e.preventDefault();pages.forEach(p=>p.classList.remove('active'));
...
document.querySelectorAll('.plan-switch').forEach(s=>{
 ...
 s.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>{
  ...
  b.classList.add('active');day.querySelector('#'+b.dataset.target).classList.add('active');
 }));
});
document.querySelectorAll('textarea[data-note]').forEach(el=>{
 const k='note:'+el.dataset.note;el.value=localStorage.getItem(k)||'';
 el.addEventListener('input',()=>localStorage.setItem(k,el.value));
});

// Shopping list
const defaultItems=[
 {id:crypto.randomUUID(),name:'柯南限定周邊',category:'動漫周邊',place:'USJ／讀賣電視台',note:'看到限定商品先確認是否限購',done:false},
 {id:crypto.randomUUID(),name:'任天堂限定商品',category:'動漫周邊',place:'Nintendo OSAKA／USJ',note:'依現場庫存決定',done:false},
 {id:crypto.randomUUID(),name:'日本藥妝',category:'藥妝美妝',place:'心齋橋',note:'集中最後一天補買',done:false},
 {id:crypto.randomUUID(),name:'大阪伴手禮',category:'食品伴手禮',place:'心齋橋／梅田',note:'確認保存期限與行李空間',done:false}
];
...
window.editItem=id=>{
 ...
 x.name=name.trim()||x.name;x.place=place.trim();x.note=note.trim();save();
}
form.addEventListener('submit',e=>{
 e.preventDefault();
 const fd=new FormData(form);
 const name=fd.get('name').trim();if(!name)return;
 items.unshift({id:crypto.randomUUID(),name,category:fd.get('category'),place:fd.get('place').trim(),note:fd.get('note').trim(),done:false});
 form.reset();save();
});
