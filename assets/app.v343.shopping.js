...
const BEAUTY_IMPORT_VERSION="beauty-products-2026-08-07-v2";
...
function beautyKey(v){return String(v||"").toLowerCase().replace(/\s+/g,"").replace(/[（）()・／/]/g,"")}
async function importBeautyProducts(showMessage=true){
  edits.shopping=Array.isArray(edits.shopping)?edits.shopping:[];
  const ids=new Set(edits.shopping.map(x=>x.id));
  const names=new Set(edits.shopping.map(x=>beautyKey(x.name)));
  let added=0;
  for(const item of BEAUTY_IMPORT_ITEMS){
    if(!ids.has(item.id)&&!names.has(beautyKey(item.name))){
      edits.shopping.push({...item});
      ids.add(item.id); names.add(beautyKey(item.name)); added++;
    }
  }
  edits.shoppingImports=Array.isArray(edits.shoppingImports)?edits.shoppingImports:[];
  if(!edits.shoppingImports.includes(BEAUTY_IMPORT_VERSION))edits.shoppingImports.push(BEAUTY_IMPORT_VERSION);
  persistLocal();
  if(tripId&&editToken&&!readonly){
    try{await saveCloud()}catch(e){console.error(e)}
  }
  renderShopping();
  const status=document.getElementById("beautyImportStatus");
  if(status)status.textContent=added?`已新增 ${added} 項`:"7 項商品已在清單中";
  if(showMessage)alert(added?`已新增 ${added} 項商品`:"這 7 項商品已經在清單中，未重複新增。");
  return added;
}
window.importBeautyProducts=importBeautyProducts;
...
async function init(){
  try{
    ...
    await importBeautyProducts(false);
...
$("#addPrepItem").onclick=addPrep;$("#addShopping").onclick=addShopping;$("#importBeautyProducts").onclick=()=>importBeautyProducts(true);
init();
