...
import {SUPABASE_URL,SUPABASE_KEY,STORAGE_BUCKET} from "./config.v201.js";
...
let edits={version:1,itemOverrides:{},hiddenItems:[],addedItems:[],hotelImage:"",selectedDay6Plan:0,lastModified:null};
...
function renderNav(){
  const editNav=[
    ["homePage","⌂","首頁"],["todayPage","◎","今日"],["itineraryPage","▣","行程"],["mapPage","⌖","地圖"],["morePage","•••","更多"]
  ];
...
function openEditor(title,fields,onSave){
  $("#editTitle").textContent=title;$("#editFields").innerHTML=fields.map(f=>`<div class="field"><label>${esc(f.label)}</label>${f.type==="textarea"?`<textarea name="${f.name}">${esc(f.value||"")}</textarea>`:`<input name="${f.name}" value="${esc(f.value||"")}"></div>`).join("");
  $("#editDialog").showModal();$("#editForm").onsubmit=e=>{e.preventDefault();onSave(Object.fromEntries(new FormData(e.target)));$("#editDialog").close()}
}
function renderMap(){
...
function renderAll(){renderNav();renderDashboard();renderHotel();renderTabs();renderDay(activeDay);renderMap();renderToday();renderCloudPanels();applyReadonly()}
